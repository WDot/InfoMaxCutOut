...
#from joint_seg_class_unet import JointSegClassUnet
...
def train(args, io):
    ...
    if not args.baseline:
        mask_model = MaskModel(device=device,num_classes=train_dataset.num_classes,doCutout=args.cutout).to(device)

    #odel.cuda()

    lossFunc = nn.DataParallel(Loss(device).cuda())
    #if not args.baseline:
    #    mask_model.cuda()
    ...
    if args.cutmix:
        cutmix = nn.DataParallel(CutMix(device=device).cuda())
    ...
    #            {'params': mask_model.parameters()},
    #            {'params': model.parameters()},
    #        ], lr=args.lr,  weight_decay=1e-4, momentum=args.momentum, nesterov=True)
    ...
    for epoch in range(args.epochs):
        ...
        for data, labels, fitz, noise in train_loader:
            ...
            with torch.cuda.amp.autocast():
                ...
                outputs,_ = model(x)
            #print('{0} {1}'.format(labels.shape,outputs.shape))
            loss = cal_loss(outputs,labels) + iicloss
            #else:
             #       x = data
            #outputs = model(data)
            #loss = lossFunc(segout,labels)
            #loss = cal_loss(outputs,labels)
            loss = loss.mean()# / args.gradaccum
            #loss.backward()

            #opt.step()
            #print(loss.detach().cpu().numpy())
            scaler.scale(loss).backward()
            #if ((count / batch_size) % args.gradaccum == 0):
        ...
        if np.mean(mean_test_accs) >= best_test_acc:
            ...
            torch.save(model.state_dict(), 'checkpoints/{0}/models/model.t7'.format(args.exp_name))
            if not args.baseline:
                torch.save(mask_model.state_dict(), 'checkpoints/{0}/models/mask_model-{1}.t7'.format(args.exp_name,epoch))

...
class CutOut(nn.Module):
    ...
    def forward(self,x):
        ...
        cutSizes = torch.squeeze((x.shape[2]/2)*torch.ones(lambdaVal.shape,device=self.device)).type(torch.int64)
        ...
        for i in range(batch_size):
            ...
            mask[i,:,startIdx:endIdx,startIdx:endIdx] = self.eps #Maskout
        return mask